...
public class PhotonServerSocket {
    ...
    private static boolean currentlyInExecution = false;
    ...
    private void Decode(String code) {
        ...
        if (code.contains(":")) {
            ...
            if (players.length == 2) {
                ...
                // Base hit logic
                if (Integer.parseInt(target) == 43) {   //someone on red hit green base
                    if (!greenBaseHitToggle && greenBaseHitterCode == -1) {
                        ...
                        if(!redBaseHitToggle && redBaseHitterCode == -1){   //only one player over both teams can have B so check other base for hits
                            pas.addB(1, Integer.parseInt(shooter));
                        }
                    }
                } else if (Integer.parseInt(target) == 53) {    //someone on green hit red base
                    if (!redBaseHitToggle && redBaseHitterCode == -1) {
                        ...
                        pas.logAction(response);
                        pas.updateScores(Integer.parseInt(shooter), Integer.parseInt(target), true);
                        if(!greenBaseHitToggle && greenBaseHitterCode == -1){   //only one player over both teams can have B so check other base for hits
                            pas.addB(0, Integer.parseInt(shooter));
                        }
                    }
                } else {
    ...
    public static boolean currentlyInExecution(){

        return currentlyInExecution;
